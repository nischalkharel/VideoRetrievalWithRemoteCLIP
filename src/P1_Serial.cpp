...
Tensor preprocess_frame(const Mat& bgr_frame){
    ...
    Tensor tensor = from_blob(
     ...
     .to(kFloat32)
     .div(255.0f);          // [0,255] -> [0.0, 1.0]

    // Step 5: Normalize with CLIP mean and std (all on CPU)
    // Using tensor() directly to avoid from_blob const-cast issues
    Tensor mean_t = tensor::from_blob((void*)MEAN, {3}, kFloat).clone().view({3, 1, 1});
    Tensor std_t  = tensor::from_blob((void*)STD,  {3}, kFloat).clone().view({3, 1, 1});
...
int main() {
    // Device setup
    Device device = cuda::is_available() ? Device(kCUDA) : Device(kCPU);
